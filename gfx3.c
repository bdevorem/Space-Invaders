...
int gfx_compare_color(int x, int y, int r, int g, int b)//added function to check pixel color
{
	XColor color; //to store color values, existing struct in Xlib
	XImage *gfx_image; //another existing struct, we use it to make the image for XGetPixel

	gfx_image = XGetImage(gfx_display, DefaultRootWindow(gfx_display), x, y, 1, 1, XAllPlanes(), XYPixmap);	//we get the image here 

	color.pixel = XGetPixel(gfx_image, 0, 0); //get the desired pixel value

	XQueryColor(gfx_display, gfx_colormap, &color); //get the colors of the pixel
printf("%i\n", color.red);
	if(color.red == r<<8 && color.green == g<<8 && color.blue == b<<8) return 1; //if the colors match the colors we sent into this function to check, then return 1
	else return -1;
}
